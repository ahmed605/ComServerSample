...
class MD5 : public CryptoHash<4> {
  private:
    ...
    constexpr uint32_t f_expr(int i, hash_parameters h) const {
      return ((h.f_array[i][0] | h.f_array[i][1]) ^ h.f_array[i][2]) & h.f_array[i][3] ^ h.f_array[i][4];
    }

    constexpr uint32_t f(PaddedValue_T value, int block_index, int i, hash_parameters h) const {
    ...
    constexpr hash_parameters hash_block(PaddedValue_T value, hash_parameters h, 
        hash_parameters prev_h, int block_index ,int i=0) const { 
      return i == 64 ? 
        hash_parameters { prev_h[0]+h[0], prev_h[1]+h[1], prev_h[2]+h[2], prev_h[3]+h[3] }
      : hash_block(value, {
            h[3],
